...
struct TeacherHomeworkHelpView: View {
    var body: some View {
        List {
            ...
            Section("コラムを配布する") {
                Text("コラムは、宿題とは別に配布できる読み物です。コラムページはスクロール方式なので長さは適宜調整可能です。字数制限はありません。")
                Text("配布のし方は基本的に宿題と同じです。コラムを作成・編集したあとコラムJSONとして書き出し、新着通知ファイルを生徒に送ります。")
                Text("新着通知ファイルには、最新コラム番号も入れられます。")
                Text("生徒には、コラムJSONとをAirDrop、メール、LINEなどで送ります。")
                Text("生徒はHOMEの「コラム」から「新規取得」を押して、受け取ったコラムJSONを取り込みます。")
            }
        }
